...
# Read NN from file
net = matopen("matlabNNSaves/net1x2x2x1relu.mat")
...
con_in1 = u - u_min
con_in2 = u_max - u
ineq_cons = vcat(con_in1,con_in2)
eq_cons = 1
#global S = @set con_in1[1] >= 0
#global S = @set S && con_in2[1] >= 0
#@constraint(model,con_in1[1] >= 0)
#@constraint(model,con_in2[1] >= 0)
...
# Psatz constraint
global expr = 0
#@variable(model, t[size(ineq_cons,1),1], SOSPoly(monomials(x,0)))
@variable(model, [1], SOSPoly(monomials(x,0)))
for j in 1:size(ineq_cons,1)
    global expr = expr + t[j]*ineq_cons[j]
end

if size(eq_cons,1) >= 2
eq_cons = eq_cons[2:end]
@variable(model, s[size(eq_cons,1),1])
for j in size(eq_cons,1)
    global expr = expr + s[j]*eq_cons[j]
end
end
...
if dim_out == 1
    ...
    #global status = zeros(2,1)
    for i in 1:2
        global expr = expr + c[i]*(d - v_out[1])
        @constraint(model, expr >= 0, sparsity = NoSparsity()) #MonomialSparsity(ChordalCompletion())
        @objective(model, Min, c[i]*d)
        #JuMP.optimize!(model)
        optimize!(model)
        #global status[i] = termination_status(model)
        global opt[i] = objective_value(model)
        #local f = c[i]*d
...
elseif dim_out == 2
    ...
    for i in 1:dim_poly
        ...
        local v_out2 = [cos(theta) sin(theta)]*v_out[:]
        @constraint(model, v_out2[1] - d >= 0, domain = S, maxdegree = 0)
