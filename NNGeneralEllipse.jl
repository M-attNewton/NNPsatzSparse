...
elseif dim_out == 2
    global opt = zeros(2,1)
    global f = d
    global eq_cons = vcat(eq_cons, d*d*v_out[1]*v_out[1] + e*e*v_out[2]*v_out[2] - d*d*e*e)
    global pop1 = vcat(f,con_in1)
    global pop1 = vcat(pop1,con_in2)
    global pop1 = vcat(pop1, ineq_cons)
    global pop1 = vcat(pop1, eq_cons)
    if size(eq_cons,1) >= 2
        global opt[1],sol,data = cs_tssos_first(pop1, vars, order, numeq=size(eq_cons,1), TS="block", solution=true)
        global f = e
        global eq_cons = vcat(eq_cons, opt[1]*opt[1]*v_out[1]*v_out[1] + e*e*v_out[2]*v_out[2] - opt[1]*opt[1]*e*e)
        global pop2 = vcat(f,con_in1)
        global pop2 = vcat(pop2,con_in2)
        global pop2 = vcat(pop2, ineq_cons)
        global pop2 = vcat(pop2, eq_cons)
        global opt[2],sol,data = cs_tssos_first(pop2, vars, order, numeq=size(eq_cons,1), TS="block", solution=true)
    else
        global opt[i],sol,data = cs_tssos_first(pop, vars, order, numeq=0, TS="block", solution=true)
